...
const ManProduct: React.FC<DealsProps>  = ({ selectedCategories, handleAddToWishlist,selectedCart,handleAddToCart }) => {
    const dispatch = useDispatch<AppDispatch>();
    const { user } = useSelector(selectAuthState);
    const [showModal, setShowModal] = useState(false);
    const { productsForMen,status} = useSelector(selectProductsForMen);
        ...
        <div className="carousel-item active">
          <div className="row justify-content-center g-2" >
            {productsForMen.map((item,index) => (
              <div className="col-6 col-sm-4 col-md-2" key={index} title={item?.name}>
  ...
  {selectedCategories.has(item?._id) ? (
    ...
    >
      &#9829;
                    ...
                    <p className="custom-card-text">Rs. {item?.price}
